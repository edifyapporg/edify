...
describe ::Members::DuplicateDismissal do
  ...
  describe "pair-ordering invariant" do
    # Create in order so ids are deterministic (first has the lower id).
    let!(:first) { create_member("Aaa, One") }
    let!(:second) { create_member("Bbb, Two") }

    it "rejects a pair whose member_a_id is not less than member_b_id" do
      swapped = described_class.new(unit: unit, member_a: second, member_b: first, dismissed_by: 1)
      expect(swapped).not_to be_valid
    end

    it "rejects a member paired with itself" do
      self_pair = described_class.new(unit: unit, member_a: first, member_b: first, dismissed_by: 1)
      expect(self_pair).not_to be_valid
    end
  end
