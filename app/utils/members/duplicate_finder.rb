module Members
  ...
  class DuplicateFinder
    # @param unit [Unit]
    # @return [Array<Members::DuplicatePair>]
    def self.call(unit)
      new(unit).call
    end
    ...
    # @return [Array<Members::DuplicatePair>]
    def call
      # Members without a birthdate are excluded: a nil birthdate is an absence of
      # data, not a match, so grouping them together would flag unrelated people.
      unit.members.where.not(birthdate: nil).to_a
          .group_by(&:birthdate)
          .each_value
    ...
    # Transliterate first so accented spellings normalize (e.g. "Muñoz" -> "munoz")
    # and match their unaccented drift instead of fragmenting into "mu"/"oz".
    # @return [Array<String>]
    def tokenize(part)
      ActiveSupport::Inflector.transliterate(part.to_s).downcase.gsub(/[^a-z\s-]/, " ").split(/[\s-]+/).compact_blank
    end
